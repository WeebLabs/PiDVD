...
static const char *const mode_v[]   = { "AUTO \xc2\xb7 LAST DISC", "PAL",
                                        "NTSC" };
...
static const struct {
    ...
    [UI_SET_MODE]   = { "PICKER MODE",    mode_v,   3 },
...
static int *field(ui_settings_t *s, int row)
{
    ...
    case UI_SET_MODE:   return &s->picker_mode;
...
void ui_settings_load(ui_settings_t *s)
{
    ...
    s->last_standard = 1;  /* picker defaults to PAL until a disc plays */
    ...
    if (f) {
        ...
        while (fgets(line, sizeof(line), f)) {
            ...
            *eq = 0;
            ...
            else if (!strcmp(line, "mode"))    s->picker_mode = atoi(v) % 3;
...
void ui_settings_save(const ui_settings_t *s)
{
    ...
    if (f) {
        fprintf(f, "theme=%d\nlayout=%d\nmode=%d\naudio=%d\ndim=%d\n"
                   "last_std=%d\nlast_disc=%s\n",
                s->theme, s->layout, s->picker_mode, s->audio_out,
                s->attract_dim, s->last_standard, s->last_disc);
        fclose(f);
    }
    if (mnt) {
        (void)system("sync");
