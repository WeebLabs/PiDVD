...
static const char *const saver_v[]  = { "OFF", "WARP STARFIELD" };
...
static const struct {
    ...
    [UI_SET_SAVER]  = { "SCREENSAVER",    saver_v,  2 },
    ...
    if (f) {
        ...
        while (fgets(line, sizeof(line), f)) {
            ...
            *eq = 0;
            ...
            else if (!strcmp(line, "saver"))   s->saver = atoi(v) % 2;
...
void ui_settings_save(const ui_settings_t *s)
{
    ...
    if (f) {
        fprintf(f, "theme=%d\nlayout=%d\naudio=%d\ndim=%d\nsaver=%d\n"
                ...
                s->saver, s->comp_filter, s->last_standard, s->last_disc);
        fclose(f);
    }
    if (mnt) {
        (void)system("sync");
